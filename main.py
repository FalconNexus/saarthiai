# =================================================================
# Saarthi AI v1.0 - FIRST PROTOTYPE
# =================================================================
...
class EnhancedAIBrain:
    ...
    def get_self_introduction(self, query):
        ...
        if any(word in q for word in ['about you', 'yourself', 'who are you', 'apne bare mein', 'tum kaun ho']):
            return (
                ...
                "I am integrated with an **Arduino** board, allowing me to translate your voice commands like 'move forward' into physical actions. "
    ...
    def process_command(self, command, set_caption_callback):
        ...
        # 2. MOVEMENT CHECK
        for cmd_phrase, arduino_cmd in self.movement_commands.items():
            if cmd_phrase in command:
                ...
                return responses.get(arduino_cmd, "Command executed!"), arduino_cmd, 'HAPPY'
...
class HardwareController:
    ...
    def connect(self, port=None):
        try:
            ...
            self.arduino = serial.Serial(port, self.baud_rate, timeout=self.command_timeout)
            time.sleep(2)
...
class IntegratedSaarthiAI:
    def __init__(self):
        ...
        pygame.display.set_caption("Saarthi AI v3.9 - Fixed Version")
    ...
    def _process_and_respond(self, command):
        response, arduino_command, response_state = self.brain.process_command(command, self.caption_system.set_text)
        
        self._set_state(response_state) 
        
        if arduino_command:
            self.hardware.send_command(arduino_command)
    ...
    def run(self):
        while self.running:
            ...
            # Smaller, cuter status display
            arduino_status = "🔗" if self.hardware.connected else "❌"
            internet_status = "🌐" if self.brain.search_enabled else "📴"
            status_text = f"{arduino_status} Arduino | {internet_status} Internet"
